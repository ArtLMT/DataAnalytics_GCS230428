...
for (col in numeric_cols) {
  print(
    ggplot(df_task2, aes_string(x = col)) +
...
for (col in numeric_cols) {
  p <- ggplot(df_task2, aes_string(x = col)) +
    geom_histogram(bins = 15, fill = "blue", color = "black", alpha = 0.7) +
    labs(title = paste("Histogram of", col, "before cleaning"),
         x = col,
         y = "Count") +
...
## 2.1 Missing value
From the metadata in the Pima info file, some values should not be zero, because it’s biologically impossible.
Those include
Glucose
BloodPressure
SkinThickness	
...
```{r}
df_mean_imputed <- df_clean  # make a copy
...
# 1. Generate small overview histograms
plot_list <- list()
for (col in numeric_cols) {
  p <- ggplot(df, aes_string(x = col)) +
    geom_histogram(bins = 15, fill = "blue", color = "black", alpha = 0.7) +
    labs(title = paste("Before:", col), x = col, y = "Count") +
    theme_minimal() +
    theme(plot.title = element_text(size = 7)) # Small title for overview
  plot_list[[col]] <- p
}

# Arrange small histograms in a grid
grid.arrange(grobs = plot_list[1:4], ncol = 2, top = "Small Overview: Before Cleaning")
if (length(numeric_cols) > 4) {
  grid.arrange(grobs = plot_list[5:length(numeric_cols)], ncol = 2, top = "Small Overview: Before Cleaning (cont.)")
}

# 2. Generate side-by-side comparison histograms
for (col in numeric_cols) {
  # Before cleaning
  p_before <- ggplot(df, aes_string(x = col)) +
    geom_histogram(bins = 15, fill = "lightgray", color = "black", alpha = 0.7) +
    labs(title = paste("Before Data", " Cleaning"), x = col, y = "Count") +
    theme_minimal()
  ...
  grid.arrange(p_before, p_after, ncol = 2, 
               top = paste("Histogram of ", col, " Before and after cleaning"))
}
```
```{r}
p_after <- ggplot(df_imputed, aes_string(x = col)) +
